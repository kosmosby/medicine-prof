...
class cbhangoutBlogTable extends OrderedTable
{
	...
	{
		$map	=	array( 'user' => 'created_by', 'category' => 'catid', 'hangout_intro' => 'introtext', 'hangout_full' => 'fulltext' );
	...
	public function load( $id = null )
	{
		...
		if ( ! $article->get( 'id' ) ) {
			...
			if ( ! $this->get( 'catid' ) ) {
				$this->set( 'catid', (int) $plugin->params->get( 'hangout_k2_category_default', null ) );
			}
	...
	public function bind( $array, $ignore = '', $prefix = null )
	{
		...
		if ( $bind ) {
			...
			$this->set( 'catid', (int) ( ( $plugin->params->get( 'hangout_category_config', 1 ) || $isModerator ) ? Get::get( $array, 'category', $this->get( 'catid' ), GetterInterface::INT ) : $this->get( 'catid', $plugin->params->get( 'hangout_k2_category_default', null ) ) ) );
...
class cbhangoutModel
{
	...
	static public function getHangoutTotal( $where, $viewer, $user, /** @noinspection PhpUnusedParameterInspection */ $plugin )
	{
		...
		$categories			=	cbhangoutModel::getCategoriesList( true );
	...
	static public function getCategoriesList( $raw = false )
	{
		...
		if ( ! isset( $cache ) ) {
			$plugin						=	cbhangoutClass::getPlugin();
			$sectionid					=	$plugin->params->get( 'hangout_k2_section', null );
