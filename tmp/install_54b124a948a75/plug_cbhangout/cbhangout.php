...
if (file_exists($_CB_framework->getCfg('absolute_path').'/components/com_comprofiler/plugin/user/plug_cbhangout/language/'.$_CB_framework->getCfg( 'lang' ).'.php')){
        include($_CB_framework->getCfg('absolute_path').'/components/com_comprofiler/plugin/user/plug_cbhangout/language/'.$_CB_framework->getCfg( 'lang' ).'.php');
}else{
        include($_CB_framework->getCfg('absolute_path').'/components/com_comprofiler/plugin/user/plug_cbhangout/language/default.php');
}

$_PLUGINS->loadPluginGroup( 'user' );
$_PLUGINS->registerFunction( 'onAfterDeleteUser', 'deleteHangout', 'cbhangoutPlugin' );

/**
 * Class cbhangoutClass
...
class cbhangoutClass
{
	...
	static public function getModel( $cms = null, $include = true )
	{
		...
		if ( ! isset( $cache[$cms] ) ) {
			...
			if ( ! $cms ) {
				$cms				=	$plugin->params->get( 'hangout_model', 2 );
			}
			...
			if ( $include ) {
				...
				$model->class		=	new cbhangoutModel();
			}
	...
	static public function getPlugin( )
	{
		...
		if ( ! isset( $plugin ) ) {
			$plugin						=	$_PLUGINS->getLoadedPlugin( 'user', 'cbhangout' );
 ...
 * Tab for CB Hangout
 */
class cbhangoutTab extends cbTabHandler
{
	/**
	 * Labeller for title:
...
class cbhangoutPlugin extends cbPluginHandler
{
	/**
