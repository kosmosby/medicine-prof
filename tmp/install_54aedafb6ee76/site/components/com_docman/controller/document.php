...
class ComDocmanControllerDocument extends ComKoowaControllerModel
{
    public function __construct(KObjectConfig $config)
    {
        ...
        $this->addCommandCallback('before.add'  , '_checkSubmittedFolder');
        $this->addCommandCallback('before.edit' , '_checkSubmittedFolder');
    ...
    public function findReferrer(KControllerContextInterface $context)
    {
        ...
        if (isset($menu->query['view']) && $menu->query['view'] !== 'list') {
     ...
     * @param KControllerContextInterface $context
     */
    protected function _checkSubmittedFolder(KControllerContextInterface $context)
    {
        $data = $context->request->data;

        // Only run it if storage_path is local and being changed
        if (isset($data->storage_path) && !preg_match('#[a-z0-9]:/#i', $data->storage_path))
        {
            $page = JFactory::getApplication()->getMenu()->getItem($this->getRequest()->query->get('Itemid', 'int'));

            if ($page->params->get('upload_folder'))
            {
                $folder = trim($page->params->get('upload_folder'), '/');

                if (strpos($data->storage_path, $folder) !== 0)
                {
                    unset($data->storage_path);
                    unset($data->storage_path_file);
                }
            }
        }
    }
    ...
    protected function _checkDownloadLink(KControllerContextInterface $context)
    {
        ...
        if ($this->getRequest()->getFormat() === 'html' && $query->view === 'document' && $this->getView()->getLayout() === 'default')
        {
            $menu = JFactory::getApplication()->getMenu()->getActive();

            if ($menu->params->get('document_title_link') === 'download'
                    && in_array($menu->query['view'], array('list', 'userlist', 'filteredlist', 'document'))
            ) {
