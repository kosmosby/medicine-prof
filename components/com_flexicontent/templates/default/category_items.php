		...
		<tr>
			<?php if ( $buttons_exists || $comments_non_zero || $show_title || count($item->css_markups) ) : ?>
				<th id="flexi_title" scope="col">
                                        <?php echo JText::_("FLEXI_DRUGS_TITLE");?>
...
<?php foreach ($columns as $name => $label) : ?>
   ...
   $link_title='';
   if ($name=='created_by') {
      // FIRST TRY community builder
      //if ($mainframe->getParams('com_comprofiler')) {
         $link = JRoute::_('index.php?option=com_comprofiler&task=userprofile&user='.$item->created_by);
         $link_title = JText::_('Author Profile');
      //}
      
      // SECOND TRY standard Joomla Contact Details
      /*else
      {
         $query = "SELECT id FROM #__contact_details WHERE user_id=". $item->created_by;
         $db = & JFactory::getDBO();
         $db->setQuery($query);
         $contact_id = $db->loadResult();
         if ($contact_id) {
            $link = JRoute::_('index.php?option=com_contact&view=contact&id='.$contact_id);
            $link_title = JText::_('Contact Details');
         }
      }*/
   }
   ?>
   ...
   <td>
      <?php echo ($link) ? "<a href='$link' alt=='$link_title'>" : ""; ?>
