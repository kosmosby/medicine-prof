...
<?php if ( $use_search || $use_filters ) : /* BOF search and filters block */ ?>
	...
	?>
<script src="<?php echo JUri::base();?>/media/jui/js/chosen.jquery.min.js"></script>
<link rel="stylesheet" href="<?php echo JUri::base();?>/media/jui/css/chosen.css">
			...
			<span class="<?php echo $filter_container_class; ?> fc_filter_text_search fc_odd">
				...
				<script type="text/javascript">
					...
					function change_search_filter(sel){
						...
						if(myForm.elements["filter_22[]"].selectedIndex != undefined){myForm.elements["filter_22[]"].selectedIndex = 0;}
						//if(myForm.elements["filter_21"].selectedIndex != undefined){myForm.elements["filter_21"].selectedIndex = 0;}
                                                if(myForm.elements["filter_24[]"].selectedIndex != undefined){myForm.elements["filter_24[]"].selectedIndex = 0;}
						
						if(sel.value == '1' || sel.value == 'val3'){
				...
				<?php if ($show_search_label==1) : ?>
					<div class="divfcsel" style="width:20% !important;">
					<select class="fc_autosubmit_exclude" style="font-size:14px; width:200px;" name="where_s" onchange="change_search_filter(this)">
						...
						<option value="val2" <?php if(isset($_GET['filter_19'])) echo "selected"; ?>>По заболеванию</option>
                                                <option value="val3" <?php if(isset($_GET['filter_21'])) echo "selected"; ?>>По производителю</option>
                                                <option value="val4" <?php if(isset($_GET['filter_22'])) echo "selected"; ?>>По веществу</option>
                                                
                                                <option value="val5" <?php if(isset($_GET['filter_24'])) echo "selected"; ?>>По АТС</option>
                                        </select>       
					</div>
					
                                <script type="text/javascript">
                                    jQuery(".fc_autosubmit_exclude").chosen({disable_search_threshold: 10});
                                </script>    
				<?php endif; ?>
				
				<span class="fc_filter_html" style="width:70% !important;">
					<span id="fat1" <?php if(!isset($_GET['filter']) && (isset($_GET['filter_15'])||isset($_GET['filter_19'])||isset($_GET['filter_21'])||isset($_GET['filter_222'])||isset($_GET['filter_24']))) echo "style='display:none' "?>>
					<input type="<?php echo $search_autocomplete==2 ? 'hidden' : 'text'; ?>" class="<?php echo $text_search_class; ?>"
						data-fc_label_text="<?php echo $text_search_label; ?>" name="filter"
                                                id="<?php echo $form_id; ?>_filter" value="<?php echo $text_search_val;?>"  style="font-size:14px !important;"/>
