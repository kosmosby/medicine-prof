...
{	
	...
	jQuery.post(siteUrl+"index.php?option=com_awdwall&task=ajaxstatuslink", { txt: value },
	function(data) {		
		var data = jQuery.parseJSON(data);
		var xmlHttp=GetXmlHttpObject();		
		if(data.url){
			document.getElementById("awd_link").value = data.url; 
			ajaxLinkStatusUpload(siteUrl+'index.php?option=com_awdwall&task=addlink&wuid='+data.wuid);				
		}else{
			document.getElementById("status_loading").style.display = 'none';
		}
	}, "html");	
