...
class CategoryTable extends OrderedTable
{
	 ...
	 * @return string
	 */
	public function canvas( $thumbnail = false, $html = true )
	{
		global $_CB_framework, $_PLUGINS;
		...
		if ( $canvas && $html ) {
			$canvas			=	'<div style="background-image: url(' . htmlspecialchars( $canvas ) . ')" class="cbImgCanvas' . ( $thumbnail ? ' cbThumbCanvas' : ' cbFullCanvas' ) . ' gjCanvas' . ( $this->get( 'canvas' ) ? ' gjCanvasCustom' : ' gjCanvasDefault' ) . '"></div>';
		}
	 ...
	 * @return string
	 */
	public function logo( $thumbnail = false, $html = true, $linked = false )
	{
		global $_CB_framework, $_PLUGINS;
		...
		if ( $logo ) {
			if ( $html ) {
				$logo		=	'<img alt="' . htmlspecialchars( CBTxt::T( 'Logo' ) ) . '" src="' . htmlspecialchars( $logo ) . '" class="cbImgPict' . ( $thumbnail ? ' cbThumbPict' : ' cbFullPict' ) . ' gjLogo' . ( $this->get( 'logo' ) ? ' gjLogoCustom' : ' gjLogoDefault' ) . ' img-thumbnail" />';
			}
