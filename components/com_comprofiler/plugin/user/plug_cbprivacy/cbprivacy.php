...
$_PLUGINS->registerFunction( 'onAftercommitFieldDataSave', 'fieldCommitSave', 'cbprivacyPlugin' );
	...
	static public function getPrivacyInput( $name, $value = '0', $attributes = null )
	{
		...
		if ( ! $loaded++ ) {
			$js			=	"$( '.cbPrivacyInput' ).cbselect({"
						...
						.		"}"
						.		"var display = $( this ).cbselect( 'container' ).find( '.ms-choice > span' );"
						.		"if ( value == 0 ) {"
						.			"display.html( '" . addslashes( CBTxt::T( 'PRIVACY_ICON_PUBLIC', '[icon] Public', array( '[icon]' => '<span class="fa fa-globe"></span>' ) ) ) . "' );"
						.		"} else if ( value == 1 ) {"
						.			"display.html( '" . addslashes( CBTxt::T( 'PRIVACY_ICON_USERS', '[icon] Users', array( '[icon]' => '<span class="fa fa-user"></span>' ) ) ) . "' );"
						.		"} else if ( value == 99 ) {"
						.			"display.html( '" . addslashes( CBTxt::T( 'PRIVACY_ICON_PRIVATE', '[icon] Private', array( '[icon]' => '<span class="fa fa-lock"></span>' ) ) ) . "' );"
						.		"} else if ( value == 2 ) {"
						.			"display.html( '" . addslashes( CBTxt::T( 'PRIVACY_ICON_CONNECTIONS', '[icon] Connections', array( '[icon]' => '<span class="fa fa-arrows-h"></span>' ) ) ) . "' );"
						.		"} else {"
						.			"display.html( '" . addslashes( CBTxt::T( 'PRIVACY_ICON_CUSTOM', '[icon] Custom', array( '[icon]' => '<span class="fa fa-gear"></span>' ) ) ) . "' );"
						.		"}"
						.	"}).change();";
		...
		return moscomprofilerHTML::selectList( $options, $name . '[]', 'class="btn btn-default btn-sm cbPrivacyInput" multiple="multiple"' . ( $attributes ? ' ' . $attributes : null ), 'value', 'text', $value, 0, false, false, false );
	}
...
class cbprivacyPrivacyTable extends Table
{
	...
	public function store( $updateNulls = false )
	{
		...
		if ( ! $new ) {
			$_PLUGINS->trigger( 'privacy_onBeforeUpdatePrivacy', array( &$this ) );
		} else {
		...
		if ( ! $new ) {
			$_PLUGINS->trigger( 'privacy_onAfterUpdatePrivacy', array( $this ) );
		} else {
...
class cbprivacyClosedTable extends Table
{
	...
	public function store( $updateNulls = false )
	{
		...
		if ( ! $new ) {
			$_PLUGINS->trigger( 'privacy_onBeforeUpdateClosed', array( &$this ) );
		} else {
		...
		if ( ! $new ) {
			$_PLUGINS->trigger( 'privacy_onAfterUpdateClosed', array( $this ) );
		} else {
