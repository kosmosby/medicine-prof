...
class CBplug_cbgroupjive extends cbPluginHandler
{
	...
	private function showCategory( $id, $user )
	{
		...
		$limitstart				=	$_CB_framework->getUserStateFromRequest( 'gj_category_limitstart{com_comprofiler}', 'gj_category_limitstart' );
		$search					=	$_CB_framework->getUserStateFromRequest( 'gj_category_search{com_comprofiler}', 'gj_category_search' );
		$where					=	null;
		...
		$pageNav->setInputNamePrefix( 'gj_category_' );
	...
	private function showGroups( $mode, $user )
	{
		...
		switch ( (int) $mode ) {
			...
			case 3:
				if ( ! $user->get( 'id' ) ) {
					cbRedirect( $_CB_framework->viewUrl( 'login', false, array( 'return' => base64_encode( $_CB_framework->pluginClassUrl( $this->element, false, array( 'action' => 'groups', 'func' => 'my' ) ) ) ) ), CBTxt::T( 'Please Login or Register to view your group invites.' ) );
				}
		...
		if ( ( ( ! $isModerator ) && ( $mode == 0 ) ) || ( $mode == 2 ) ) {
			$query				.=	"\n LEFT JOIN " . $_CB_database->NameQuote( '#__groupjive_users' ) . " AS u"
								.	' ON u.' . $_CB_database->NameQuote( 'user_id' ) . ' = ' . (int) $user->get( 'id' )
								.	' AND u.' . $_CB_database->NameQuote( 'group' ) . ' = g.' . $_CB_database->NameQuote( 'id' );
		}

		if ( ( ( ! $isModerator ) && ( $mode == 0 ) ) || ( $mode == 3 ) ) {
			$query				.=	"\n LEFT JOIN " . $_CB_database->NameQuote( '#__groupjive_invites' ) . " AS i"
								.	' ON i.' . $_CB_database->NameQuote( 'group' ) . ' = g.' . $_CB_database->NameQuote( 'id' )
								.	' AND i.' . $_CB_database->NameQuote( 'accepted' ) . ' = ' . $_CB_database->Quote( '0000-00-00 00:00:00' )
								.	' AND ( ( i.' . $_CB_database->NameQuote( 'email' ) . ' = ' . $_CB_database->Quote( $user->get( 'email' ) )
	...
	private function showGroupMessage( $id, $user )
	{
		...
		$input['message']		=	'<textarea id="message" name="message" class="form-control input-block required" rows="10"' . $messageTooltip . '>' . htmlspecialchars( $this->input( 'post/message', null, GetterInterface::STRING ) ) . '</textarea>';
	...
	private function showGroupUsers( &$group, $user )
	{
		...
		$limitstart				=	$_CB_framework->getUserStateFromRequest( 'gj_group_users_limitstart{com_comprofiler}', 'gj_group_users_limitstart' );
		$search					=	$_CB_framework->getUserStateFromRequest( 'gj_group_users_search{com_comprofiler}', 'gj_group_users_search' );
		$where					=	null;
		...
		$pageNav->setInputNamePrefix( 'gj_group_users_' );
		...
		$input['search']		=	'<input type="text" name="gj_group_users_search" value="' . htmlspecialchars( $search ) . '" onchange="document.gjGroupUsersForm.submit();" placeholder="' . htmlspecialchars( CBTxt::T( 'Search Users...' ) ) . '" class="form-control" />';
