...
class cbantispamClass
{
	...
	static public function getUserBlocks( $user = null, $ipAddress = null )
	{
		...
		if ( ! isset( $cache[$userId][$ipAddress] ) ) {
...
class cbantispamCaptcha
{
	 ...
	 * @param string $mode
	 */
	public function __construct( $cookieName = null, $mode = null )
	{
		global $_PLUGINS;

		$this->plugin		=	$_PLUGINS->getLoadedPlugin( 'user', 'cbantispam' );
		$this->params		=	$_PLUGINS->getPluginParams( $this->plugin );

		if ( ! $cookieName ) {
			$cookieName		=	uniqid( 'cbantispamCaptcha' );
		}

		$this->name			=	$cookieName;

		$defaultMode		=	$this->params->get( 'captcha_mode', 'internal' );
	 ...
	 */
	static public function getInstance( $cookieName = null, $mode = null )
	{
		static $cache			=	array();
		...
		if ( ! isset( $cache[$cookieName] ) ) {
			$cache[$cookieName]	=	new cbantispamCaptcha( $cookieName, $mode );
		}
	...
	/**
	 * Stores data to the captcha session
	 *
	 * @param array $data
	 */
	...
	public function getSession()
	{
		...
		if ( ! isset( $cache[$this->name] ) ) {
			...
			$cookie						=	$sessions->get( $this->name );

			if ( $cookie ) {
				$cache[$this->name]		=	$cookie;
			}
		}
	...
	public function getCaptchaCode()
	{
		if ( ! $this->code ) {
			...
			if ( $session ) {
				$this->code		=	Get::get( $session, 'code', null, GetterInterface::STRING );
			}
		...
		switch( $this->mode ) {
			...
			default:
				...
				if ( strpos( $font, '/' ) === false ) {
					$absPath		=	$_PLUGINS->getPluginPath( $this->plugin );
				...
				$image				=	'<img src="' . $_CB_framework->pluginClassUrl( $this->plugin->get( 'element' ), true, array( 'action' => 'captcha', 'func' => $this->mode, 'id' => $this->name ), 'raw', 0, true ) . '" id="' . htmlspecialchars( $this->name ) . 'Image" alt="' . htmlspecialchars( CBTxt::T( 'Captcha' ) ) . '"' . ( $height || $width ? ' style="' . ( $height ? 'height: ' . (int) $height . 'px;' : null ) . ( $width ? 'width: ' . (int) $width . 'px;' : null ) . '"' : null ) . ' class="cbantispamCaptchaImage" />';
				break;
		}
	...
	public function getCaptchaAudio( $hidden = false )
	{
		...
		switch( $this->mode ) {
			...
			case 'question':
			default:
				$audio		=	'<audio src="' . $_CB_framework->pluginClassUrl( $this->plugin->get( 'element' ), true, array( 'action' => 'captcha', 'func' => 'audio', 'id' => $this->name ), 'raw', 0, true ) . '" id="' . htmlspecialchars( $this->name ) . 'AudioFile" type="audio/mpeg" class="cbantispamCaptchaAudioFile"' . ( $hidden ? ' style="display: none !important;"' : null ) . '></audio>';
				break;
		}
	...
	public function getCaptchaInput( $size = null, $attributes = null )
	{
		switch( $this->mode ) {
			...
			default:
				if ( ! $size ) {
					$size		=	(int) $this->params->get( 'captcha_internal_size', 20 );
				...
				$input			=	'<input type="text" name="' . htmlspecialchars( $this->name ) . '" id="' . htmlspecialchars( $this->name ) . '" value=""' . ( $size ? ' size="' . (int) $size . '"' : null ) . ' class="required form-control"' . $attributes . '>';
				break;
		}
	...
	public function getCaptchaHTMLImage( $audio = null, $refresh = null, $height = null, $font = null )
	{
		...
		switch( $this->mode ) {
			case 'recaptcha':
				...
				if ( ! $JS_loaded++ ) {
					$language		=	$this->params->get( 'captcha_recaptcha_lang', 'en' );
					...
					$js				=	"var loadCBAntiSpamRecaptcha = function() {"
									...
									.				"sitekey: '" . addslashes( $this->params->get( 'captcha_recaptcha_site_key' ) ) . "',"
									.				"theme: '" . addslashes( $this->params->get( 'captcha_recaptcha_theme', 'light' ) ) . "'"
									.			"});"
									.		"}"
									.	"};";
				...
				$html				=	'<div id="' . htmlspecialchars( $this->name ) . '" class="cbantispamRecaptcha"></div>';
				break;
			case 'internal':
			case 'question':
			default:
				...
				$audio				=	( $this->mode == 'question' ? false : ( $audio === null ? $this->params->get( 'captcha_internal_audio', 1 ) : $audio ) );
				$refresh			=	( $refresh === null ? $this->params->get( 'captcha_internal_refresh', 1 ) : $refresh );
				...
				if ( $audio || $refresh ) {
					...
					if ( $refresh ) {
						$html		.=			'<div>'
									.				'<span id="' . htmlspecialchars( $this->name ) . 'Refresh" class="cbantispamCaptchaRefresh fa fa-refresh" style="vertical-align: top; margin-bottom: 3px; cursor: pointer;" title="' . htmlspecialchars( CBTxt::T( 'Refresh Captcha' ) ) . '"></span>'
									.			'</div>';
					}

					if ( $audio ) {
						$html		.=			'<div>'
									.				'<span id="' . htmlspecialchars( $this->name ) . 'Audio" class="cbantispamCaptchaAudio fa fa-volume-up" style="vertical-align: top; cursor: pointer;" title="' . htmlspecialchars( CBTxt::T( 'Listen to Captcha' ) ) . '"></span>'
	...
	public function getCaptchaInputValue()
	{
		switch( $this->mode ) {
			case 'recaptcha':
				$value	=	Get::get( $_REQUEST, 'g-recaptcha-response', null, GetterInterface::STRING );
				break;
			case 'internal':
			case 'question':
			default:
				$value	=	Get::get( $_REQUEST, $this->name, null, GetterInterface::STRING );
				break;
		}
...
class cbantispamBlockTable extends Table
{
	...
	public function store( $updateNulls = false )
	{
		...
		if ( ! $new ) {
			$_PLUGINS->trigger( 'antispam_onBeforeUpdateBlock', array( &$this ) );
		} else {
		...
		if ( ! $new ) {
			$_PLUGINS->trigger( 'antispam_onAfterUpdateBlock', array( $this ) );
		} else {
	...
	public function getExpire() {
		...
		if ( ! isset( $cache[$id] ) ) {
			if ( $this->get( 'duration' ) ) {
				$expire		=	$_CB_framework->getUTCDate( 'Y-m-d H:i:s', $_CB_framework->getUTCTimestamp( strtolower( $this->get( 'duration' ) ), $this->get( 'date' ) ) );
			} else {
	...
	public function isExpired()
	{
		...
		if ( ! isset( $cache[$id] ) ) {
			$cache[$id]	=	( $this->get( 'duration' ) && ( $_CB_framework->getUTCNow() >= $_CB_framework->getUTCTimestamp( strtolower( $this->get( 'duration' ) ), $this->get( 'date' ) ) ) ? true : false );
		}
...
class cbantispamWhitelistTable extends Table
{
	...
	public function store( $updateNulls = false )
	{
		...
		if ( ! $new ) {
			$_PLUGINS->trigger( 'antispam_onBeforeUpdateWhitelist', array( &$this ) );
		} else {
		...
		if ( ! $new ) {
			$_PLUGINS->trigger( 'antispam_onAfterUpdateWhitelist', array( $this ) );
		} else {
...
class cbantispamLogTable extends Table
{
	...
	public function store( $updateNulls = false )
	{
		...
		if ( ! $new ) {
			$_PLUGINS->trigger( 'antispam_onBeforeUpdateLog', array( &$this ) );
		} else {
		...
		if ( ! $new ) {
			$_PLUGINS->trigger( 'antispam_onAfterUpdateLog', array( $this ) );
		} else {
	...
	public function block()
	{
		...
		if ( $this->get( 'user_id' ) || $this->get( 'ip_address' ) ) {
			...
			if ( ! $block->get( 'id' ) ) {
				...
				$block->set( 'date', $_CB_framework->getUTCDate() );
			...
			if ( $_PLUGINS->is_errors() || $user->getError() ) {
				...
				if ( $this->params->get( 'login_share', 0 ) ) {
					...
					$query					=	'SELECT COUNT(*)'
											...
											.	"\n WHERE " . $_CB_database->NameQuote( 'user_id' ) . " = " . (int) $user->get( 'id' );
					if ( $timeframe ) {
						$query				.=	"\n AND " . $_CB_database->NameQuote( 'date' ) . " >= " . $_CB_database->Quote( $_CB_framework->getUTCDate( 'Y-m-d H:i:s', $timeframe ) );
					}
					$_CB_database->setQuery( $query );
