...
class cbconsultationsconsultationTable extends OrderedTable
{
	...
	public function load( $id = null )
	{
		...
		if ( ! $article->get( 'id' ) ) {
			...
			if ( ! $this->get( 'catid' ) ) {
				$this->set( 'catid', (int) $plugin->params->get( 'consultation_k2_category_default', null ) );
			}
		...
		$map			=	array( 'user' => 'created_by', 'category' => 'catid', 'consultation_intro' => 'introtext', 'consultation_full' => 'fulltext' );
	...
	public function bind( $array, $ignore = '', $prefix = null )
	{
		...
		if ( $bind ) {
			...
			$this->set( 'catid', (int) ( ( $plugin->params->get( 'consultation_category_config', 1 ) || $isModerator ) ? Get::get( $array, 'category', $this->get( 'catid' ), GetterInterface::INT ) : $this->get( 'catid', $plugin->params->get( 'consultation_k2_category_default', null ) ) ) );
...
class cbconsultationsModel
{
	 ...
	 * @return int
	 */
	static public function getconsultationsTotal( $where, $viewer, $user, /** @noinspection PhpUnusedParameterInspection */ $plugin )
	{
		global $_CB_database;
