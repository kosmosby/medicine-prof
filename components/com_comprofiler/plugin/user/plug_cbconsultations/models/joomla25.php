...
class cbconsultationsconsultationTable extends OrderedTable
{
	...
	public function load( $id = null )
	{
		...
		if ( ! $article->get( 'id' ) ) {
			...
			if ( ! $this->get( 'catid' ) ) {
				$this->set( 'catid', (int) $plugin->params->get( 'consultation_j_category_default', null ) );
			}

			if ( ! $this->get( 'state' ) ) {
				$this->set( 'state', (int) $plugin->params->get( 'consultation_approval', 0 ) );
			}
		...
		$map			=	array( 'user' => 'created_by', 'published' => 'state', 'category' => 'catid', 'consultation_intro' => 'introtext', 'consultation_full' => 'fulltext' );
	...
	public function bind( $array, $ignore = '', $prefix = null )
	{
		...
		if ( $bind ) {
			...
			$this->set( 'introtext', Get::get( $array, 'consultation_intro', $this->get( 'introtext' ), GetterInterface::HTML ) );
			$this->set( 'fulltext', Get::get( $array, 'consultation_full', $this->get( 'fulltext' ), GetterInterface::HTML ) );
			$this->set( 'catid', (int) ( ( $plugin->params->get( 'consultation_category_config', 1 ) || $isModerator ) ? Get::get( $array, 'category', $this->get( 'catid' ), GetterInterface::INT ) : $this->get( 'catid', $plugin->params->get( 'consultation_j_category_default', null ) ) ) );
			$this->set( 'state', (int) ( ( ( ! $plugin->params->get( 'consultation_approval', 0 ) ) || $isModerator ) ? Get::get( $array, 'published', $this->get( 'state' ), GetterInterface::INT ) : $this->get( 'state', $plugin->params->get( 'consultation_approval', 0 ) ) ) );
	...
	public function check( )
	{
		...
		if ( $this->get( 'title' ) == '' ) {
			...
			return false;
		} elseif ( ! in_array( $this->get( 'catid' ), cbconsultationsModel::getCategoriesList( true ) ) ) {
	...
	public function store( $updateNulls = false )
	{
		...
		if ( ! $new ) {
			...
			$_PLUGINS->trigger( 'cbconsultations_onBeforeUpdateconsultation', array( &$this, &$article, $user, $plugin ) );
		} else {
			$article->set( 'created', $_CB_framework->getUTCDate() );

			$_PLUGINS->trigger( 'cbconsultations_onBeforeCreateconsultation', array( &$this, &$article, $user, $plugin ) );
		}
		...
		if ( ! $new ) {
			$_PLUGINS->trigger( 'cbconsultations_onAfterUpdateconsultation', array( $this, $article, $user, $plugin ) );
		} else {
			$_PLUGINS->trigger( 'cbconsultations_onAfterCreateconsultation', array( $this, $article, $user, $plugin ) );
		}
	...
	{
		$category	=	new cbconsultationsCategoryTable( $this->_db );
...
class cbconsultationsCategoryTable extends OrderedTable
{
...
class cbconsultationsModel
{
	 ...
	 * @return int
	 */
	static public function getconsultationsTotal( $where, $viewer, $user, $plugin )
	{
		global $_CB_database;

		$section	=	$plugin->params->get( 'consultation_j_section', null );
	 ...
	 */
	static public function getconsultations( $paging, $where, $viewer, $user, $plugin )
	{
		global $_CB_database;
		...
		return $_CB_database->loadObjectList( null, 'cbconsultationsconsultationTable', array( $_CB_database ) );
	}
