...
$_PLUGINS->registerFunction( 'onAfterDeleteUser', 'deleteconsultations', 'cbconsultationsPlugin' );

/**
 * Class cbconsultationsClass
...
class cbconsultationsClass
{
	...
	static public function getModel( $cms = null, $include = true )
	{
		...
		if ( ! isset( $cache[$cms] ) ) {
			...
			if ( ! $cms ) {
				$cms				=	$plugin->params->get( 'consultation_model', 2 );
			}
			...
			if ( $include ) {
				...
				$model->class		=	new cbconsultationsModel();
			}
	...
	{
		$categories					=	cbconsultationsClass::getModel()->class->getCategoriesList( true );
		...
		return cbconsultationsClass::getModel()->type;
	}
	...
	static public function getPlugin( )
	{
		...
		if ( ! isset( $plugin ) ) {
			$plugin						=	$_PLUGINS->getLoadedPlugin( 'user', 'cbconsultations' );
 ...
 */
class cbconsultationsTab extends cbTabHandler
{
	/**
	 * Labeller for title:
	...
	public function getTabTitle( $tab, $user, $ui, $postdata )
	{
		...
		$total		=	cbconsultationsModel::getconsultationsTotal( null, $viewer, $user, $plugin );
...
class cbconsultationsPlugin extends cbPluginHandler
{
	/**
