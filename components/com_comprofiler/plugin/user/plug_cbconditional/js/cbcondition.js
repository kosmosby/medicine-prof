	...
	var methods = {
		init: function( options ) {
			return this.each( function () {
				...
				if ( ! $( $this ).length ) {
					// Looks like condition was called on an element that doesn't exist; maybe it's top level?
					$.cbcondition( options );
					return;
				}
				...
				cbcondition.target.on( 'keyup change', cbcondition.changeHandler ).change();
				...
				cbcondition.element.on( 'cloned.cbcondition', function( e, oldId ) {
					...
					target.off( 'keyup change', cbcondition.changeHandler );
		...
		destroy: function() {
			...
			cbcondition.target.off( 'keyup change', cbcondition.changeHandler );
		...
		if ( element.length && element.hasClass( 'cbDisplayDisabled' ) ) {
			if ( element.hasClass( 'cbTabPane' ) ) {
				...
				if ( element.closest( '.cbSingleCntrl' ).length ) {
					element.closest( '.cbSingleCntrl' ).show();
				} else if ( element.closest( '.cbSnglCtrlLbl' ).length ) {
					element.closest( '.cbSnglCtrlLbl' ).show();
				} else if ( element.parent( 'label' ).length ) {
					element.parent( 'label' ).show();
				} else {
					element.show();
				}
			} else {
			...
			element.removeClass( 'cbDisplayDisabled' );
	...
	function hideElement( cbcondition, condition, element ) {
		if ( element.length && ( ! element.hasClass( 'cbDisplayDisabled' ) ) ) {
			if ( element.hasClass( 'cbTabPane' ) ) {
				...
				if ( element.closest( '.cbSingleCntrl' ).length ) {
					element.closest( '.cbSingleCntrl' ).hide();
				} else if ( element.closest( '.cbSnglCtrlLbl' ).length ) {
					element.closest( '.cbSnglCtrlLbl' ).hide();
				} else if ( element.parent( 'label' ).length ) {
			...
			element.addClass( 'cbDisplayDisabled' );
