<?php
/**
 * @version    $Id: cbpaidAES128.php 1619 2013-01-09 23:58:29Z beat $
 ...
 *
 * Autor: Jose Manuel Busto Lopez
 * Version: 1.1
 * Fecha: 04-05-2007
 * Licencia: Lesser General Public License (LGPL)
 *
 * Copyright (C) 2007  Jose Manuel Busto Lopez
...
class cbpaidAES128
{
	...
	/* Definicion de la S-box inversa (para el descifrado) */
	private $Si = array(
	...
	/* Definicion de los valores de la funcion rcon */
	private $rcon = array(
	...
	private $Nr = 10; //Numero de rondas 128b=>10
	private $Nb = 4; //Numero de columnas en el state (siempre 4)
	private $Nk = 4; //Longitud de la clave 128b=>4
...
/////////////////////////////////////////////////////////////////////////////
	 ...
	 * @access private
	 *
	 * @param  string[]  $rk  Array de caracteres (4x4) con la clave i-esima
	 */
...
/////////////////////////////////////////////////////////////////////////
	 ...
	 *
	 * @param  boolean  $hex    Si es verdadero devuelve los valores en hexadecimal
	 * @param  boolean  $debug  Si es verdadero muestra informacion de debug
	 */
	 ...
	 *
	 * @param  string  $hash   key  Clave de cifrado longitud igual a 16 caracteres.
	 * @return array           Matriz de claves para cifrar/descifrar.
	 */
	public function makeKey( $hash )
	{
		...
		for ( $j = 0; ( $j < $this->Nk ) && ( $t < ( $this->Nr + 1 ) * $this->Nb ); $j++, $t++ )
			for ( $i = 0; $i < 4; $i++ )
				$rk[(int) ( $t / $this->Nb )][$i][$t % $this->Nb] = $tk[$i][$j];
	 ...
	 * @access private
	 *
	 * @param  string  $in   Datos a cifrar (longitud 128 bits).
	 * @param  string  $key  Array de claves de cifrado (generadas con makeKey())
	 * @return string        La cadena de datos cifrada.
	 */
	private function blockEncrypt( $in, $key )
	{
	 ...
	 * @access private
	 *
	 * @param  string  $in   Datos a descifrar (longitud 128 bits).
	 * @param  array   $key  Array de claves de cifrado (generadas con makeKey())
	 * @return string        La cadena de datos descifrada.
	 */
	private function blockDecrypt( $in, $key )
	{
	 ...
	 * @access private
	 *
	 * @param  string  $in    Data to encrypt
	 * @param  array   $key   Key generated by makeKey())
	 * @return string         Encrypted text
	 */
	/** @noinspection PhpUnusedPrivateMethodInspection */
	private function encryptECB( $in, $key )
	{
		$padding = 16 - ( strlen( $in ) % 16 );
		if ( $padding != 16 ) {
			$in .= str_repeat( "\0", $padding );
		}
		$len = strlen( $in );
		$out = null;
		for ( $i = 0; $i < $len; $i = $i + 16 ) {
			$out .= $this->blockEncrypt( substr( $in, $i, 16 ), $key );
		}
		return $out;
	}
	 ...
	 */
	/** @noinspection PhpUnusedPrivateMethodInspection */
	private function decryptECB( $in, $key )
	{
		$padding = strlen( $in ) % 16;
		if ( $padding != 0 ) {
			trigger_error( 'encrypted not multiple of 16 bytes.' );
		}
		$len = strlen( $in );
		$out = null;
		for ( $i = 0; $i < $len; $i = $i + 16 ) {
			$out .= $this->blockDecrypt( substr( $in, $i, 16 ), $key );
		}
		return rtrim( $out, "\0" );
	}

	/**
	 * Random string of $stringLength binary bytes generator
	 *
	 * @param  int     $stringLength  number of chars
	 * @return string                 password
	 */
	public function makeRandomString( $stringLength )
	{
...
// $stat[]		=	$_CB_framework->getCfg( 'secret' );
		...
		for ( $i = 0; $i < $stringLength; $i++ ) {
			$rndString .= chr( mt_rand( 0, 255 ) );
		}
		return $rndString;
	}

	/**
	 * Does a Xor over $res with $in
	 * (supposing that strlen( $in ) <= strlen( $res ) )
	 * if $in is smaller than $res, $res is truncated to the length of $in
	 *
	 * @access private
	 *
	 * @param string $res
	 ...
	 * @param  string  $in    Data to encrypt
	 * @param  array   $key   Key generated by makeKey()
	 * @param  string  $iV    Initial vector (16 bytes)
	 * @return string         Encrypted text with prepended iV (so cyphertext output is 16 bytes longer than plaintext)
	 */
	public function encrypt( $in, $key, $iV = null )
	{
		if ( $iV === null ) {
			$iV = $this->makeRandomString( 16 );
		}
		$len = strlen( $in );
		$padding = 16 - ( $len % 16 );
		if ( $padding == 16 ) {
		...
		$out = $iV;
		$outBlk = $iV;
		for ( $i = 0, $n = $len + $padding; $i < $n; $i = $i + 16 ) {
			$outBlk = $this->blockEncrypt( $outBlk, $key );
			$this->_Xor( $outBlk, substr( $in, $i, 16 ) );
			$out .= $outBlk;
		}
		return $out;
	}
	 ...
	 * @access private
	 *
	 * @param  string  $in  Cyphertext (with prepended iV)
	 * @param  array   $key Array de claves de cifrado (generadas con makeKey())
	...
	public function decrypt( $in, $key )
	{
		$len = strlen( $in );
		$out = null;
		$outBlk = substr( $in, 0, 16 );
		for ( $i = 16; $i < $len; $i = $i + 16 ) {
			$outBlk = $this->blockEncrypt( $outBlk, $key );
			$this->_Xor( $outBlk, substr( $in, $i, 16 ) );
			$out .= $outBlk;
			$outBlk = substr( $in, $i, 16 );
		}
		return rtrim( $out, "\0" );
	}
...
//////////////////////////////////////////////////////////////////////
	...
	/**
	 * shows an integer in hexadecimal (ECHOs)
	 *
	 * @param  string  $texto
	 */
