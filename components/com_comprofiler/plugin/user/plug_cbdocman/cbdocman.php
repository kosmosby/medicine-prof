...
global $_PLUGINS, $_LANG, $_CB_framework;

if (file_exists($_CB_framework->getCfg('absolute_path').'/components/com_comprofiler/plugin/user/plug_cbdocman/language/'.$_CB_framework->getCfg( 'lang' ).'.php')){
        include($_CB_framework->getCfg('absolute_path').'/components/com_comprofiler/plugin/user/plug_cbdocman/language/'.$_CB_framework->getCfg( 'lang' ).'.php');
}else{
        include($_CB_framework->getCfg('absolute_path').'/components/com_comprofiler/plugin/user/plug_cbdocman/language/default.php');
}
...
class cbdocmanClass
{
	...
	static public function getPlugin( )
	{
		...
		if ( ! isset( $plugin ) ) {
			$plugin						=	$_PLUGINS->getLoadedPlugin( 'user', 'cbdocman' );
...
cbdocmanClass::getModel();

/**
 * Class cbdocmanTab
 * Tab for CB Docman
 */
class cbdocmanTab extends cbTabHandler
{
	/**
	 * Labeller for title:
	...
	public function getDisplayTab( $tab, $user, $ui )
	{
		...
		$plugin				=	cbdocmanClass::getPlugin();
		$model				=	cbdocmanClass::getModel();
		$viewer				=	CBuser::getMyUserDataInstance();

		cbdocmanClass::getTemplate( 'tab' );

		$limit				=	(int) $this->params->get( 'tab_limit', 15 );
		$limitstart			=	$_CB_framework->getUserStateFromRequest( 'tab_docman_limitstart{com_comprofiler}', 'tab_docman_limitstart' );
		$filterSearch		=	$_CB_framework->getUserStateFromRequest( 'tab_docman_search{com_comprofiler}', 'tab_docman_search' );
		$where				=	null;
		...
		$total				=	cbdocmanModel::getDocmanTotal( $where, $viewer, $user, $plugin );
		...
		$pageNav->setInputNamePrefix( 'tab_docman_' );

		$rows				=	cbdocmanModel::getDocman( ( $this->params->get( 'tab_paging', 1 ) ? array( $pageNav->limitstart, $pageNav->limit ) : null ), $where, $viewer, $user, $plugin );

		$input				=	array();
		//$input['search']	=	'<input type="text" name="tab_docman_search" value="' . htmlspecialchars( $filterSearch ) . '" onchange="document.articleForm.submit();" placeholder="' . htmlspecialchars( CBTxt::T( 'Search Articles...' ) ) . '" class="form-control" />';
		...
		$return				=	'<div id="cbArticles" class="cbArticles' . ( $class ? ' ' . htmlspecialchars( $class ) : null ) . '">'
							...
							.			HTML_cbdocmanTab::showDocmanTab( $rows, $pageNav, $searching, $input, $viewer, $user, $model, $tab, $plugin )
							.		'</div>'
							.	'</div>';
