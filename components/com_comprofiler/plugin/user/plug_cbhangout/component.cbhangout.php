...
/**
 * Class CBplug_cbhangout
 * CB Components-type class for CB Blogs
 */
class CBplug_cbhangout extends cbPluginHandler
{
	/**
	...
	public function showBlog( $id, $user, $model, $plugin )
	{
		...
		$profileUrl				=	$_CB_framework->userProfileUrl( $row->get( 'user', $user->get( 'id' ) ), false, 'cbhangoutTab' );
                ...
                $query = 'SELECT user FROM #__comprofiler_plugin_hangout_join'
                        .' WHERE hangout_id = '.$id;
                $_CB_database->setQuery( $query );
		$usrs = $_CB_database->loadResultArray();
		...
		cbhangoutClass::getTemplate( 'blog_show' );

		HTML_cbhangoutBlog::showBlog( $row, $user, $model, $plugin, $usrs);
	}
	...
	private function saveBlogEdit( $id, $user, $model, $plugin )
	{
		...
		if ( $canAccess ) {
			if ( $plugin->params->get( 'hangout_captcha', 0 ) && ( ! $row->get( 'id' ) ) && ( ! $cbModerator ) ) {
			...
			if ( $new && ( ! $row->get( 'published' ) ) && $plugin->params->get( 'approval_notify', 1 ) && ( ! $cbModerator ) ) {
				...
				$extraStrings		=	array(	'site_name' => $_CB_framework->getCfg( 'sitename' ),
												...
												'hangout_id' => (int) $row->get( 'id' ),
												'hangout_title' => $row->get( 'title' ),
												'hangout_intro' => $row->get( 'hangout_intro' ),
												'hangout_full' => $row->get( 'hangout_full' ),
												'hangout_created' => $row->get( 'hangout_created' ),
												'hangout_user' => (int) $row->get( 'user' ),
												'hangout_url' => cbhangoutModel::getUrl( $row ),
												'hangout_tab_url' => $_CB_framework->viewUrl( 'userprofile', false, array( 'user' => (int) $row->get( 'user' ), 'tab' => 'cbhangoutTab' ) ),
				...
				$message			=	$cbUser->replaceUserVars( CBTxt::T( '[user] created [hangout_title] and requires <a href="[hangout_tab_url]">approval</a>!' ), false, true, $extraStrings, false );
			...
			cbRedirect( $profileUrl, $_LANG['Hangout saved successfully!'] );
		} else {
			cbRedirect( $profileUrl, CBTxt::T( 'Not authorized.' ), 'error' );
		}
	}
	...
	private function stateBlog( $state, $id, $user, /** @noinspection PhpUnusedParameterInspection */ $model, $plugin )
	{
		...
		if ( $row->load( (int) $id ) ) {
			if ( $row->get( 'id' ) && ( Application::User( (int) $user->get( 'id' ) )->isGlobalModerator() || ( ( $row->get( 'user' ) == $user->get( 'id' ) ) && ( ! $plugin->params->get( 'hangout_approval', 0 ) ) ) ) ) {
				$canAccess			=	true;
			}
		}
		...
		if ( $canAccess ) {
			...
			cbRedirect( $profileUrl, $_LANG['Hangout state saved successfully!'] );
		} else {
			cbRedirect( $profileUrl, CBTxt::T( 'Not authorized.' ), 'error' );
		}
	}
	...
	private function deleteBlog( $id, $user, /** @noinspection PhpUnusedParameterInspection */ $model, /** @noinspection PhpUnusedParameterInspection */ $plugin )
	{
        ...
        private function joinConf( $id, $user, /** @noinspection PhpUnusedParameterInspection */ $model, /** @noinspection PhpUnusedParameterInspection */ $plugin )
	{
		global $_CB_framework, $_CB_database;

		$query = 'INSERT IGNORE INTO #__comprofiler_plugin_hangout_join(user,hangout_id)'
                        .' VALUES('.(int)$user->get( 'id' ).', '.$id.')';
                $_CB_database->setQuery( $query );
		$_CB_database->query();
                cbRedirect(cbSef( 'index.php?option=com_comprofiler&view=pluginclass&plugin=cbhangout&action=hangout&func=show&id=' . (int) $id, true ));
	}
