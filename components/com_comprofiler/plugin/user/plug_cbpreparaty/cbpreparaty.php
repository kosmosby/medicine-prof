...
if (file_exists($_CB_framework->getCfg('absolute_path').'/components/com_comprofiler/plugin/user/plug_cbpreparaty/language/'.$_CB_framework->getCfg( 'lang' ).'.php')){
        include($_CB_framework->getCfg('absolute_path').'/components/com_comprofiler/plugin/user/plug_cbpreparaty/language/'.$_CB_framework->getCfg( 'lang' ).'.php');
}else{
        include($_CB_framework->getCfg('absolute_path').'/components/com_comprofiler/plugin/user/plug_cbpreparaty/language/default.php');
}
...
class cbpreparatyClass
{
	...
	static public function getModel( $cms = null, $include = true )
	{
		...
		if ( ! isset( $cache[$cms] ) ) {
			...
			if ( in_array( $cms, array( 1, 4, 6 ) ) && checkJversion() >= 2 ) {
				...
				$model				=	cbpreparatyClass::getModel( $cms, false );
			}
			...
			if ( $include ) {
				...
				$model->class		=	new cbpreparatyModel();
			}
	...
	static public function getPlugin( )
	{
		...
		if ( ! isset( $plugin ) ) {
			$plugin						=	$_PLUGINS->getLoadedPlugin( 'user', 'cbpreparaty' );
...
cbpreparatyClass::getModel();

/**
 * Class cbarticlesTab
 * Tab for CB Articles
 */
class cbpreparatyTab extends cbTabHandler
{
	/**
	 * Labeller for title:
	...
	public function getTabTitle( $tab, $user, $ui, $postdata )
	{
		...
		$total		=	cbpreparatyModel::getPreparatyTotal( null, $viewer, $user, $plugin );
