...
class oseMscAddonActionMemberDirectory
{
	public static function getDirectoryItem($params = array())
	{
    	...
    	$query = " SELECT luv.* "
    			." FROM  `#__oselic_cs_keys_view` AS luv"
    			." WHERE luv.user_id = {$member_id}";
    			;
		$db->setQuery($query);
		
		$keyInfo = oseDB::loadItem('obj');
		
		
    	$query = " SELECT COUNT(*) FROM `#__osemsc_directory` AS directory "
    			." WHERE directory.company_id = {$keyInfo->company_id}";
    			;
		$db->setQuery($query);
		...
		if($db->loadResult() < 1)
		{
			...
			$params['member_id'] = $keyInfo->member_id;
			
			$updated = oseRegistry::call('msc')->runAddonAction('join.directory.remove',$params);
			$updated = oseRegistry::call('msc')->runAddonAction('join.directory.save',$params);
	...
	public static function getLocationItem($params = array())
	{
    	...
    	$query = " SELECT c.* FROM `#__oselic_cs_company_view` AS c"
    			...
    			." WHERE u.user_id = {$user_id} AND c.location_id = {$location_id}"
    			;
		$db->setQuery($query);
		//oseExit($db->_sql);
		$item = oseDB::loadItem();
    ...
    public static function updateLocationInfo($params = array())
    {
    	...
    	$query = " SELECT * FROM `#__oselic_cs_company_view` AS mcv "
				." INNER JOIN  `#__oselic_cs_keys_view` AS luv ON luv.company_id = mcv.company_id"
    			." WHERE luv.user_id = {$user->id}"
    			." ORDER BY mcv.location_id"
    			;
    	$db->setQuery($query);
