...
class oseMscAddonActionRegisterPaymentM
{
	...
	*/
	...
	function getMethod()
	{
		...
		{
			$methods[] = array('id'=> 1, 'value'=>'paypal', 'text' => 'Paypal');
		}

		if(!empty($pConfig->enable_gco))
		{
			$methods[] = array('id'=> 2, 'value'=>'gco', 'text' => 'Google Checkout');
		}

		if(!empty($pConfig->enable_2co))
		{
			$methods[] = array('id'=> 3, 'value'=>'2co', 'text' => '2 Checkout');
		}
		...
		if(!empty($pConfig->enable_cc))
		{
			...
			foreach( $cc_methods as $cc_method)
			{
				...
				{
					case('authorize'):
						$methods[] = array('id'=> 4, 'value'=>$cc_method, 'text' => 'Credit Card');
					break;
		...
		if(!empty($pConfig->enable_eway))
		{
			$methods[] = array('id'=> 5, 'value'=>'eway', 'text' => 'eWay');
		}
		$result = array();
