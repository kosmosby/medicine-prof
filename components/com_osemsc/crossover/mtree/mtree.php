<?php
/**
 * @version		$Id: mtree.php 1434 2012-04-17 13:14:00Z cy $
 * @package		Mosets Tree
...
function showTree_cache( $cat, $limitstart, $option, $my ) {
	...
	if ( isset($cat->cat_published) && $cat->cat_published == 0 && $cat_id > 0 ) {
		...
		if ( ($cat_id == 0 || $cat->cat_usemainindex == 1) && $mtconf->getTemParam('numOfSubcatsToDisplay',3) != '0' && !empty($cat_ids)) {
			# Get all sub-cats
		...
		if( ($cat_id == 0 || $cat->cat_usemainindex == 1) && is_numeric($mtconf->getTemParam('numOfLinksToDisplay',3)) && $mtconf->getTemParam('numOfLinksToDisplay',3)!=0 && !empty($cat_ids)) {
...
function searchby( $option )
{
	...
	# Set title
	$title = JText::sprintf( 'Search By Title', $customfield->caption, $value );
	setTitle($title);
...
function search( $option ) {
	...
	if(!empty($search_cat) && empty($searchword)) {
		$app->redirect( JRoute::_("index.php?option=$option&task=listcats&cat_id=$search_cat&Itemid=".$items[0]->id) );
	}
	...
	# Set title
	$title = JText::sprintf( 'SEARCH RESULTS FOR KEYWORD', $searchword );
	setTitle($title);
...
function advsearch2( $option ) {
	...
	# Set title
	$title = JText::_( 'Advanced search results' );
	setTitle($title);
...
function listalpha( $cat_id, $alpha, $limitstart, $option ) {
	...
	# Set title
	$title = JText::sprintf( 'List alpha by listings and cats', strtoupper($alpha), $cat_name );
	setTitle($title);
...
function listlisting( $cat_id, $option, $my, $task, $limitstart ) {
	...
	# Set title
	$title = $listListing->getTitle() . $cat_name;
	setTitle($title);
...
function viewowner( $user_id, $limitstart, $option ) {
	...
	if( !empty($owner) ) {
		$title = JText::sprintf( 'Listing by', $owner->username );
		setTitle($title);
...
function viewusersreview( $user_id, $limitstart, $option ) {
	...
	if( count($owner) == 1 && $mtconf->get('show_review') ) {
		# Set title
		$title = JText::sprintf( 'Reviews by', $owner->username );
		setTitle($title);
...
function viewusersfav( $user_id, $limitstart, $option ) {
	...
	if( count($owner) == 1 && $mtconf->get('show_favourite')) {
		# Set title
		$title = JText::sprintf( 'Favourites by', $owner->username );
		setTitle($title);
...
function viewgallery( $link_id, $option ) {
	...
	if($link === false)	{
		...
		# Set title
		$title = JText::sprintf( 'Gallery2', $link->link_name );
		setTitle($title);
...
function viewimage( $img_id, $option ) {
	...
	if($link === false)	{
		...
		# Set title
		$title = JText::sprintf( 'Image2', $link->link_name );
		setTitle($title);
...
function printlink( $link_id, $option ) {
	...
	if (empty($link)) {
		...
		$title = JText::_( 'Print2' ) . $link->link_name;
		setTitle($title);
...
function report( $link_id, $option ) {
	...
	if($link === false) {
		...
		$title = JText::_( 'Report2' ) . $link->link_name;
		setTitle($title);
...
function claim( $link_id, $option ) {
	...
	if($link === false) {
		...
		$title = JText::_( 'Claim listing' ) . ': ' . $link->link_name;
		setTitle($title);
...
function writereview( $link_id, $option ) {
	...
	if($link === false) {
		...
		$title = JText::_( 'Review' ) ." ". $link->link_name;
		setTitle($title);
...
function rate( $link_id, $option ) {
	...
	$link = loadLink( $link_id, $savantConf, $fields, $params );
	$title = JText::_( 'Rate' ) . $link->link_name;
	setTitle($title);
...
function reportreview( $rev_id, $option ) {
	...
	if( $mtconf->get('user_report_review') == -1 || ($mtconf->get('user_report_review') == 1 && $my->id == 0) ) {
		...
		if( $review->link_id > 0 ) {
			...
			$title = JText::_( 'Report review' ) . ': ' . $review->rev_title;
			setTitle($title);
...
function replyreview( $rev_id, $option ) {
	...
	if( isset($review) && $review->link_id > 0 && $my->id > 0 && $mtconf->get('owner_reply_review') ) {
		...
		if( $link->user_id == $my->id ) {
			$title = JText::_( 'Reply review' ) . ': ' . $review->rev_title;
			setTitle($title);
...
function recommend( $link_id, $option ) {
	...
	if($link === false) {
		...
		$title = JText::_( 'Recommend' ) ." ". $link->link_name;
		setTitle($title);
...
function contact( $link_id, $option ) {
	...
	if ( empty($link) ) {
		...
		$title = JText::_( 'Contact2' ) . $link->link_name;
...
function editlisting( $link_id, $option ) {
	...
	{
		if( $cat ) {
			$title = JText::sprintf( 'Add listing2', $cat->cat_name);
		} else {
			$title = JText::_( 'Add listing' );
		}
	} else {
		$title = JText::sprintf( 'Edit listing2', $link->link_name);
	}
	...
	if ( ($mtconf->get('user_addlisting') == 1 && $my->id < 1) || ($link_id > 0 && $my->id == 0) ) {
		...
		JText::script('ADD AN IMAGE');
		JText::script('REMOVE');
		JText::script('ENTER AN ADDRESS AND PRESS LOCATE IN MAP OR MOVE THE RED MARKER TO THE LOCATION IN THE MAP BELOW.');
		JText::script('LOCATE IN MAP', true);
		JText::script('LOCATING', true);
		JText::script('NOT FOUND2', true);
