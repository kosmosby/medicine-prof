<?php
/**
 * @version		$Id: mtree.tools.php 1248 2011-11-04 13:40:16Z cy $
 * @package		Mosets Tree
...
function getCatsSelectlist( $cat_id=0, &$cat_tree, $max_level=0 ) {
	...
	$sql = "SELECT *, '".$level."' AS level FROM #__mt_cats WHERE cat_published=1 && cat_approved=1 && cat_parent= " . $database->quote($cat_id) . $where." ORDER BY cat_name ASC";
	$database->setQuery( $sql );
...
function applyMambots( &$links ) {
	...
	for( $i=0; $i<count($links); $i++ ) {
		...
		$dispatcher	=& JDispatcher::getInstance();
		$results = $dispatcher->trigger('onContentPrepare', array ('com_mtree.listing', & $links[$i], & $params->params, 0));
		}
