									...
									if ($this->commentObj->can_vote)
									{
										...
										if ($this->params->get('allow_vote_down_comment', 1))
										{
											if (!$this->commentObj->voted_value)
											{
												...
												$vote_btn_html .= '<button class="vote-up btn btn-default btn-xs" title="' . JText::_('COM_JUDOWNLOAD_VOTE_UP') . '"
																	onclick="judlVoteComment(\''.$this->commentObj->id.'\', \'up\', \''.JSession::getFormToken().'\', this); return false;">
																	<i class="fa fa-thumbs-o-up"></i>
																</button>';
												$vote_btn_html .= '<button class="vote-down btn btn-default btn-xs" title="' . JText::_('COM_JUDOWNLOAD_VOTE_DOWN') . '"
							...
							<div class="comment-actions clearfix">
								...
								if($this->item->params->get('access-comment') || $this->commentObj->can_reply)
								{
									...
									<a class="btn btn-default btn-xs comment-quote" href="#"
										onclick="judlQuoteComment(<?php echo  $this->commentObj->id; ?>); return false;">
								...
								if (JUDLPROVERSION && $this->commentObj->can_subscribe)
								{
									...
									{
										?>
										<a class="judl-unsubscribe-comment btn btn-default btn-xs" href="<?php echo $this->commentObj->subscribe_link; ?>">
									...
									{
										?>
										<a class="judl-subscribe-comment btn btn-default btn-xs" href="<?php echo $this->commentObj->subscribe_link; ?>">
								...
								if ($this->commentObj->can_edit)
								{
									if ($this->commentObj->checked_out)
									{
										if ($this->commentObj->checkout_link)
										{
											...
											echo '<a class="hasTooltip btn btn-default btn-xs" title="' . $tooltip . '" href="' . $this->commentObj->checkout_link . '"><i class="fa fa-lock"></i> ' . JText::_('COM_JUDOWNLOAD_EDIT_COMMENT') . '</a>';
										}
										else
										{
											echo '<span class="btn btn-default btn-xs"><i class="fa fa-lock"> ' . JText::_('COM_JUDOWNLOAD_EDIT_COMMENT') . '</span></i>';
										}
									}
									else
									{
										?>
										<a class="btn btn-default btn-xs comment-edit" href="#" onclick="judlEditComment('<?php echo $this->commentObj->id; ?>'); return false;">
											...
											<div class="modal-content">
												...
												<div class="modal-footer">
													<a href="<?php echo $this->commentObj->link_delete; ?>"
													        class="btn btn-default btn-primary"><?php echo JText::_("COM_JUDOWNLOAD_DELETE"); ?>
													</a>
