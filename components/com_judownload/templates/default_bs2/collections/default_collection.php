			...
			if ($this->item->can_vote)
			{
				...
				if ($this->params->get('collection_allow_vote_down', 1))
				{
					if (!$this->item->voted_value)
					{
						...
						$vote_btn_html .= '<button class="vote-up btn btn-mini" title="' . JText::_('COM_JUDOWNLOAD_VOTE_UP') . '" onclick="return false;">
												<i class="fa fa-thumbs-o-up"></i>
											</button>';
						$vote_btn_html .= '<button class="vote-down btn btn-mini" title="' . JText::_('COM_JUDOWNLOAD_VOTE_DOWN') . '" onclick="return false;">
...
if ((($this->col_counter % $this->collection_columns) == 0) && (($this->index + 1) < count($this->items)))
{
	...
	<div
	class="judl-collection-row <?php echo $this->collection_row_class; ?> judl-collection-row-<?php echo $this->row_counter + 1; ?> row-fluid">
<?php
}
?>