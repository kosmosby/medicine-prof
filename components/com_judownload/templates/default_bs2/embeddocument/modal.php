...
if(!$field_display){
	$field_display = array('title', 'icon', 'introtext', 'tag');
}
?>
<div id="judl-container" class="jubootstrap component judl-container view-embeddocument">
	<div class="judownload-manager">
		<form
			action="<?php echo JRoute::_('index.php?option=com_judownload&view=embeddocument&layout=modal&tmpl=component&function=' . $function . '&' . JSession::getFormToken() . '=1'); ?>"
			...
			<div class="clearfix">
				...
				<div class="filter-select pull-left">
					<select style="margin-left: 0" name="filter_featured" class="input-medium"
						...
						<option
							value="1" <?php echo $this->state->get('filter.featured') === '1' ? "selected" : ""; ?>><?php echo JText::_('COM_JUDOWNLOAD_FEATURED'); ?></option>
						<option
							value="0" <?php echo $this->state->get('filter.featured') === '0' ? "selected" : ""; ?>><?php echo JText::_('COM_JUDOWNLOAD_UNFEATURED'); ?></option>
					</select>
			...
			<div class="filter-select row-fluid" id="options">
				<div class="span4">
					...
					<a class="btn btn-default" name="insert" id="insert" onclick="return InsertDocument('<?php echo $ename; ?>');"><?php echo JText::_('COM_JUDOWNLOAD_INSERT_ALL_SELECTED'); ?></a>
				</div>
				<div class="span4">
			...
			<table class="adminlist table table-striped table-bordered" id="document-list">
				<thead>
				<tr>
			...
			<a class="btn" name="insert" id="insert"
			   onclick="return InsertDocument('<?php echo $ename; ?>');"><?php echo JText::_('COM_JUDOWNLOAD_INSERT_ALL_SELECTED'); ?></a>
		</form>
	</div>
</div>