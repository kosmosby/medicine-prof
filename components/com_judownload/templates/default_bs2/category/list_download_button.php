...
if ($this->item->total_files > 0 || (isset($specialFields['external_link']) && $specialFields['external_link']->value != ''))
{
	if ($this->item->params->get('access-download'))
	{
		if ((isset($specialFields['license_id']) && $specialFields['license_id']->value) && (isset($specialFields['confirm_license']) && $specialFields['confirm_license']->value))
		{
			$classConfirmLicense = 'confirm-license';
			?>
			<div id="<?php echo 'judl-license-' . $this->item->id; ?>" class="modal hide fade" tabindex="-1"
			     role="dialog"
			     aria-labelledby="judl-doc-license-label" aria-hidden="true">
				<div class="modal-header">
					<button type="button" class="close" data-dismiss="modal" aria-hidden="true">&times;</button>
					<h3 id="judl-doc-license-label" class="modal-title">
						<?php echo JText::sprintf('COM_JUDOWNLOAD_LICENSE_CONFIRMATION', $specialFields['license_id']->value->title); ?>
					</h3>
				</div>
				<div class="modal-body">
					<?php echo $specialFields['license_id']->value->description; ?>
				</div>
				<div class="modal-footer">
					<a href="<?php echo $this->item->download_link; ?>"
					   id="judl-accept-license-<?php echo $this->item->id; ?>"
					   class="judl-accept-license btn btn-primary"
		...
		if ($this->item->params->get('valid-password'))
		{
			...
			<a href="<?php echo $this->item->download_link; ?>" id="<?php echo 'btn-download-' . $this->item->id; ?>"
			   class="hasTooltip btn judl-btn-download judl-accept-download <?php echo $classConfirmLicense; ?>"
		...
		else
		{
			if ($this->item->allow_enter_password)
			{
				...
				<a href="#" id="<?php echo 'btn-download-' . $this->item->id; ?>"
				   class="hasTooltip btn judl-btn-download download-unlock <?php echo $classConfirmLicense; ?>"
				   title="<?php echo JText::_('COM_JUDOWNLOAD_DOWNLOAD'); ?>">
					<i class="fa fa-download"></i>
				</a>
			<?php
			...
			else
			{
				?>
				<a href="#" id="<?php echo 'btn-download-' . $this->item->id; ?>"
				   class="hasTooltip btn judl-btn-download download-locked"
				   title="<?php echo JText::_('COM_JUDOWNLOAD_DOWNLOAD'); ?>">
					<i class="fa fa-lock"></i>
				</a>
				<div id="<?php echo 'judl-locked-alert-' . $this->item->id; ?>" class="modal hide fade"
				     tabindex="-1"
		...
		if (isset($this->item->error_msg))
		{
			...
			elseif ($this->display_download_rule_msg == "redirect")
			{
				...
				<a href="<?php echo $this->item->download_link; ?>"
				   ...
				   class="hasTooltip btn judl-btn-download"
				   title="<?php echo JText::_('COM_JUDOWNLOAD_DOWNLOAD'); ?>">
					<i class="fa fa-download"></i>
				</a>
			<?php
			...
			elseif ($this->display_download_rule_msg == "modal")
			{
				?>
				<a href="#" id="<?php echo 'btn-download-' . $this->item->id; ?>"
				   class="hasTooltip btn judl-btn-download error-rule"
				   title="<?php echo JText::_('COM_JUDOWNLOAD_DOWNLOAD'); ?>">
					<i class="fa fa-download"></i>
				</a>
