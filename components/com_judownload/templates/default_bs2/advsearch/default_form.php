		...
		$("#categories").change(function () {
			...
			$.ajax({
				...
				beforeSend: function (xhr) {
					$('#judl-field > ul > li:eq(0)').show().addClass('active');
					$('#judl-field > ul > li:gt(0)').hide().removeClass('active');

					$('#judl-field > div > div:eq(0)').show().addClass('active');
					$('#judl-field > div > div:gt(0)').removeClass('active').find('[name^="fields["]').prop("disabled", true);
				}
			}).done(function (data) {
				if (data) {
					...
					for (var i = 0; i < fieldGroupIds.length; i++) {
						if (fieldGroupIds[i] > 0) {
							$('#judl-field > ul').find('a[href="#fieldgroup-' + fieldGroupIds[i]+'"]').closest('li').show();
