...
class ComDocmanViewListHtml extends ComDocmanViewHtml
{
    ...
    protected function _fetchData(KViewContext $context)
    {
         ...
         */
        if (in_array($this->getLayout(), array('tree', 'treetable')) && !$this->getModel()->getState()->isUnique())
        {
            $first         = $this->getModel()->limit(1)->fetch();
            $state->slug   = $first->slug;
            $category_link = $this->getTemplate()->createHelper('com://admin/docman.template.helper.route')
                                ->category(array('entity' => $first), true);

            $this->getObject('response')->getHeaders()->set('Link', '<'.$category_link.'>; rel="canonical"');
        }
