...
<?php // File selection field for document forms
$local   = $this->translate('Local');
$remote  = $this->translate('Remote');
...
<div class="docman_grid">
    <div class="docman_grid__item one-whole">
        <div class="control-group">
            <label class="control-label"><?php echo $this->translate('File settings'); ?></label>
            <div class="controls" id="storage-path-container">
                <div class="input-group">
                    <span class="input-group-btn">
                        <button class="btn dropdown-toggle" type="button" data-toggle="dropdown">
                            <span data-type="<?php echo $document->storage_type ? $document->storage_type : 'file'; ?>"
                                  class="current-storage-type">
                            <?php echo $current; ?></span><span class="caret"></span>
                        </button>
                        <ul class="dropdown-menu">
                            <li>
                                <a href="#" data-type="file"><?php echo $local; ?></a>
                                <a href="#" data-type="remote"><?php echo $remote; ?></a>
                            </li>
                        </ul>
                    </span>
                    ...
                    <div class="input-group-form-control-field input-file" style="display: none;">
                        <div class="input-group">
                            <?php $folder   = dirname($local_value) !== '.' ? dirname($local_value) : '';
                            $file     = ltrim(basename(' '.strtr($local_value, array('/' => '/ '))));
                            $location = 'folder='.$folder.'&file='.$file;
                            ?>
                            <?php echo $this->helper('modal.select', array(
                                'name'  => 'storage_path_file',
                                'id' => 'storage_path_file',
                                'value' => $local_value,
                                'link'  => $this->route('option=com_docman&view=files&layout=select&tmpl=koowa&'.$location),
                                'link_text' => $document->isNew() ? $this->translate('Upload') : $this->translate('Update'),
                    ...
                    <div class="input-group-form-control-field input-remote" style="display: none;">
                        <div class="input-group">
                            <input data-rule-streamwrapper
                                   ...
                                   maxlength="512"
                                   placeholder="http://"
                                   data-streams="<?php echo htmlentities(json_encode($document->getSchemes())); ?>"
                                   name="storage_path_remote"
                                   value="<?php echo $this->escape($remote_value); ?>"
                                />
                        </div>
                        <p class="help-block"><?php echo $this->translate('Enter the remote URL in the field above') ?></p>
                    </div>
                </div>
            </div>
        </div>
