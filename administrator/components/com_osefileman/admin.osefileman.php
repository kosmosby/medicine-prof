...
if($version >= '1.6')
	{
		...
		if (empty($results))
		{
		$query= "UPDATE `#__menu` SET `alias` =  'OSE Fileman™', `path` =  'OSE Fileman™', `published`=1, `img` = '\"components/com_osefileman/favicon.ico\"'  WHERE `component_id` = ( SELECT extension_id FROM `#__extensions` WHERE element ='com_osefileman' and name ='osefileman' ) ";
		$db->setQuery($query);
		$db->query();
		}
	}
