...
class oseMscAddonActionContentHwdvideo extends oseMscAddon
{
	...
	function &getList()
	{
		...
		if($search)
		{
			...
			$query = ' SELECT hwd.id '
			.' FROM `#__hwdvidscategories` AS hwd'
			. $where
			.' ORDER BY hwd.`id`';
			$db->setQuery($query);
	...
	function changeStatus()
	{
		...
		foreach($catids as $catid)
		{
			...
			{
				$item= $content->getItem('hwdvideo', 'category', $content_id, 'msc', $msc_id, '', 'obj');
				if(empty($item))
				{
					$updated= $content->insert('hwdvideo', 'category', $content_id, 'msc', $msc_id, $newStatus);
					if(!$updated)
					{
						$result= array();
