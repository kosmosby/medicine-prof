...
class oseMscAddonActionContentK2cat extends oseMscAddon
{
	...
	function &getList()
	{
		...
		if($search)
		{
			$searchQuery = ' LOWER(k2c.`name`) LIKE '.$db->Quote('%'.$search.'%') ;
			$where[] =  $searchQuery;
	 ...
	 if ($search)
	 {
		...
		foreach($list as $item)
		{
			$obj = oseRegistry::call('content')->getInstance('msc')->getItem('k2','category',$item->id,'msc',$msc_id,null,'obj');
