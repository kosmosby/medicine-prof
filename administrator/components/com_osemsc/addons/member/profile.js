	    			                ...
	    			                ,listeners: {
	    				            	fileselected: function(fb, v){
		    				    	    		...
		    				    	    		Ext.each(item.items.items, function(subitem,i,all){
		    				    	    			if(subitem.getXType() == 'textfield' && subitem.id == ('profile_'+pid) && subitem.getValue() != '')	{
		    				    	    				...
		    				    	    				Ext.Ajax.request({
		    				    	    					...
		    				    	    					params: {task: "action",action:'member.profile.reset','file_path':path}
		    				    	    				});
		    				    	    				subitem.setValue('');
		    						    			}
		    				    				})
	    				            		...
	    				            		uploadImgWin.form.getForm().submit({
	    				            			url:'index.php?option=com_osemsc&controller=member',
	    				            			params:{task: "action",action:'member.profile.upload','pid':pid},
		...
		,items:[{
		    ...
		    ,height: 150
		  	...
		  	,store: new Ext.data.Store({
		    	...
		    	,baseParams:{task: "action",action:'member.profile.getOptions',id:pid}
	...
	oseMscAddon.profile = new Ext.FormPanel({
		...
		,buttons: [{
			...
			handler: function(){
				oseMscAddon.profile.getForm().submit({
				    ...
				    ,url: 'index.php?option=com_osemsc&controller=member'
				    ,params: {
				        task: 'action', action : 'member.profile.save',member_id: oseMemsMsc.member_id
				    }
				    ,success: function(form, action) {
		...
		,listeners: {
			render: function(p){
				Ext.Ajax.request({
					url: 'index.php?option=com_osemsc&controller=member',
					params: {task: "action",action:'member.profile.getProfile',member_id: oseMemsMsc.member_id}
					,success: function(response, opt) {
						var res = Ext.decode(response.responseText);
