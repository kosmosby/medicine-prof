...
Ext.onReady(function(){
	...
	var iterateLoadok = function(task,id,fn)	{
		...
		Ext.Ajax.request({
			...
			,params: {controller: 'authorize', task: task ,id: id}
		,timeout: 120000
	...
	Ext.select('.ose-action-edit').addListener('click',function(e,t)	{
		...
		var w = new Ext.Window({
			title: 'Edit Authorize Order Info'
			,width: 600
			,items: [{
				xtype: 'form'
				,defaults: {anchor: '90%'}
				...
				,items: [{
					...
					,fieldLabel: 'Serial Number (Subscription ID)'
					,allowBlank: false
				},{
					xtype: 'textfield'
					,name: 'order_number'
					,fieldLabel: 'Order Number(Invoice Number)'
				},{
					xtype: 'numberfield'
						,name: 'payment_price'
						,fieldLabel: 'Price'
				...
				,buttons: [{
					...
					,handler: function(btn)	{
						...
						bf.getForm().submit({
							url: 'index.php?option=com_osemsc'
							,params: {controller: 'authorize' , task: 'updateAuthorizeOrderInfo'}
				...
				,listeners: {
					render: function(f)	{
						if(id > 0)	{
							f.getForm().load({
								url: 'index.php?option=com_osemsc'
								,params: {controller: 'authorize', task: 'loadOrderInfo',id:id}
							});
						}
					}
				}
...
<form
	action="<?php echo JRoute::_('index.php?option=com_osemsc&view=authorize');?>"
	...
	<table class="adminlist">
		...
		<?php foreach ($this->items as $i => $item) : ?>
			<tr class="row<?php echo $i % 2; ?>">
				...
				<td ><?php echo $item->payment_serial_number;?></td>
				
				<td ><?php echo $item->expired_date;?></td>
				
				<td class="center">
					<div style="float:left;width:30px;height:10px;"></div>
					<div class="ose-action-edit center btns" id="ose-action-<?php echo $item->id;?>"  title="Edit"></div>
					
					<div class="ose-action-validate center btns" oid="ose-action-<?php echo $item->order_id;?>"  title="Validate"></div>
