...
class oseMscModelPaypal extends JModelList
{
	protected $items;
...
public function __construct($config = array())
	...
	function getItems()
	{
		...
		foreach ($items as $k => $item)
		{
			...
			else
			{
				...
				if(!empty($order_id))
				{
					$query = " SELECT `order_id`,`order_number`,`payment_serial_number`,`order_status`"
					." FROM `#__osemsc_order`"
	...
	protected function getListQuery()
	{
		...
		if(!empty($search) || !empty($searchid) || !empty($searchpe))
		{
			...
			$query->from('#__osemsc_member AS c');
			...
			if($search)
			{
				$query->leftJoin('#__osemsc_order_fix AS f ON f.member_id = c.id');
			...
			elseif($searchpe)
			{
				$query->innerJoin('`#__osemsc_order_fix` AS e ON e.member_id = c.id');
				// Escape the search token.
				$token	= $db->Quote('%'.$searchpe.'%');
			
				// Add the clauses to the query.
				$query->where('e.email LIKE '.$token);
			}
		...
		{
			$query->select($this->getState('list.select', 'c.*,d.username,d.name,d.email,b.order_id,b.order_number,b.payment_serial_number,b.order_status'));
			...
			$query->where("b.`payment_method` = 'paypal'");
			//$query->where("c.`msc_id` NOT IN (9)");
			$query->where("b.`order_status` IN ('confirmed','skipped')");
			$query->where("b.`payment_mode` = 'a'");
	...
	protected function populateState($ordering = null, $direction = null)
	{
		...
		$search = $this->getUserStateFromRequest($this->context.'.filter.searchpe', 'filter_searchpe');
		//$search = ($status == 'failed')?'':$search;
		$this->setState('filter.searchpe', $search);
