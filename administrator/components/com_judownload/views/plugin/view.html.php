...
class JUDownloadViewPlugin extends JUDLViewAdmin
{
	...
	protected function addToolBar()
	{
		...
		JToolBarHelper::title(JText::_('COM_JUDOWNLOAD_PAGE_' . ($checkedOut ? 'VIEW_PLUGIN' : ($isNew ? 'ADD_PLUGIN' : 'EDIT_PLUGIN'))), 'plugin-add');
		...
		if ($isNew && $this->canDo->get('core.create'))
		{
			if ($this->getLayout() == 'install')
			{
				...
				JToolBarHelper::back('JTOOLBAR_BACK', 'index.php?option=com_judownload&view=plugins');
			}
			else
			{
	...
	protected function setDocument()
	{
		...
		$checkedOut = !($this->item->checked_out == 0 || $this->item->checked_out == $userId);
		$document->setTitle(JText::_('COM_JUDOWNLOAD_PAGE_' . ($checkedOut ? 'VIEW_PLUGIN' : ($isNew ? 'ADD_PLUGIN' : 'EDIT_PLUGIN'))));
