...
$listOrder_document = $this->escape($this->state->get('list.ordering'));
$listDirn_document = $this->escape($this->state->get('list.direction'));
...
<script type="text/javascript">
    ...
    Joomla.orderTable = function (element) {
        if(element == "cat"){
            ...
            order = table.options[table.selectedIndex].value;
            if (order != '<?php echo $listOrder_document; ?>') {
                dirn = 'asc';
            } else {
                dirn = direction.options[direction.selectedIndex].value;
            }
    ...
    function listItemTask(id, task) {
		var f = document.adminForm;
		if (task == 'documents.orderdown' || task == 'documents.orderup' || task == 'documents.unpublish' || task == 'documents.publish' || task == 'documents.featured' || task == 'documents.unfeatured' || task == 'documents.checkin') {
			id = id.replace('cb', 'doc');
			var item = 'doc';
		} else {
			var item = 'cb';
		}
...
<div class="jubootstrap">
	...
	<div id="splitterContainer">
		...
		<div id="rightPane">
			<div class="inner-pane">
				<?php echo JUDownloadHelper::generateCategoryPath($cat_id, "li", true, false); ?>

				<div class="list-item">
					<form
						action="<?php echo JRoute::_('index.php?option=com_judownload&view=listcats&cat_id=' . $cat_id); ?>"
						...
						<div>
							...
							<input type="hidden" name="filter_order" value="<?php echo $listOrder_document; ?>" />
							<input type="hidden" name="filter_order_Dir" value="<?php echo $listDirn_document; ?>" />
