...
class oseMscJaccess{
	function OSEMscJacess() {
		jimport('joomla.environment.uri');
	}
	function addaclname($acl_name) {
		$db = & JFactory :: getDBO();
		$acl_name = $db->quote($db->getEscaped($acl_name));
		$query = "SELECT * FROM `#__groups`";
		$db->setQuery($query);
		$total = count($db->loadObjectList());

		$query = "SELECT * FROM `#__groups` WHERE `name` = " . $acl_name;
		$db->setQuery($query);
		$results = $db->loadObject();

		if (empty ($results)) {
			$query = "INSERT INTO `#__groups` (`id` , `name`)
								VALUES ('{$total}', {$acl_name});";
			$db->setQuery($query);
			if ($db->query()) {
				echo "1";
				exit;
			} else {
				echo JText :: _("Failed adding ACL group.");
			}
		} else {
			echo JText :: _("ACL name already exits.");
			exit;
		}
	}
		...
		if (!empty ($results)) {
			...
			foreach ($results as $result) {
				foreach ($results2 as $result2) {
					$viewarray = self :: get_viewaccess($result2->id);
	...
	function update_viewaccess($msc_id, $acl_id) {
		$cr_mscids = self :: get_viewaccess($acl_id);
		$db = & JFactory :: getDBO();
		if (empty ($cr_mscids)) {
