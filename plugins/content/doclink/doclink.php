...
class PlgContentDoclink extends JPlugin
{
     ...
     * Returns a view
     *
     * TODO: file locator template overrides are not working at the moment.
     *
    ...
    protected function _getView()
    {
        if (!self::$_view)
        {
            $template = 'file://plugins/content/doclink/view/default';
            $override = 'com://site/docman.doclink.default';

            if ($this->getObject('template.locator.factory')->createLocator($override)->locate($override.'.html')) {
                $template = $override;
            }
    ...
    protected function _getLinks(&$text)
    {
        ...
        foreach ($matches as $i => &$match)
        {
            ...
            $match->link  = preg_match('#href="(.*option=com_docman.*)"#i', $match->attributes, $temp) ? $temp[1] : null;
