...
function moduleSetTimeLeft(pref,maxtimers)
{
    if (!maxtimers) maxtimers=100;
    for(i=1;i<=maxtimers;i++) {
        el=document.getElementById(pref+i);
        if (!el) continue;
        val=el.innerHTML;
    ...
    if(i > 1) {
        window.setTimeout('moduleSetTimeLeft("'+pref+'",'+maxtimers+')',1000);
    }
}